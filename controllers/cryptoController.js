...
router.get('/catalog', async (req, res) => {
    const crypto = await cryptoService.getAll();   // ili tuk da se liinva cryptoService.getAll().lean();
...
router.get('/:cryptoId/details', async (req, res) => {
    ...
    const isOwner = crypto.owner == req.user?._id; // or crypto.owner.toString() === req.user?._id;
    const isBuyer = crypto.buyers?.some(id => id == req.user._id); // => return true or false

    // crypto.paymentMethod = paymentMethodsMap[crypto.paymentMethod]; // tova e bonua
